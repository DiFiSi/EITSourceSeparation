...
%directoy the file emdFun.m developed by Rilling and Flandrin.
%This file is available at %http://perso.ens-lyon.fr/patrick.flandrin/emdFun.html
...
%[1] Colominas MA, Schlotthauer G, Torres ME. "Improve complete ensemble emdFun: A suitable tool for biomedical signal processing" 
...
for i=1:NR
    modes_white_noise{i}=emdFun(white_noise{i});%calculates the modes of white gaussian noise
end;

for i=1:NR %calculates the first mode
    xi=x+Nstd*modes_white_noise{i}(1,:)/std(modes_white_noise{i}(1,:));
    [temp, o, it]=emdFun(xi,'MAXMODES',1,'MAXITERATIONS',MaxIter);
    temp=temp(1,:);
    aux=aux+(xi-temp)/NR;
    iter(i,1)=it;
end;
...
k=1;
aux=zeros(size(x));
es_imf = min(size(emdFun(medias(end,:),'MAXMODES',1,'MAXITERATIONS',MaxIter)));
...
while es_imf>1 %calculates the rest of the modes
    for i=1:NR
        ...
        if tamanio(1)>=k+1
            ...
            noise=Nstd*noise;
            try
                [temp,o,it]=emdFun(medias(end,:)+std(medias(end,:))*noise,'MAXMODES',1,'MAXITERATIONS',MaxIter);
            catch    
                it=0;
                temp=emdFun(medias(end,:)+std(medias(end,:))*noise,'MAXMODES',1,'MAXITERATIONS',MaxIter);
            end;
            temp=temp(end,:);
        else
            try
                [temp, o, it]=emdFun(medias(end,:),'MAXMODES',1,'MAXITERATIONS',MaxIter);
            catch
                temp=emdFun(medias(end,:),'MAXMODES',1,'MAXITERATIONS',MaxIter);
                it=0;
            end;
            temp=temp(end,:);
        end;
